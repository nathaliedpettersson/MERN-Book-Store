...
const CreateBook = () => {
    ...
    const handleSaveBook = () => {
        ...
        axios
            ...
            .catch((error) => {
                setLoading(false);
                console.log(error, 'Something went wrong while trying to post a new book.')
            })
    }
    ...
    return (
        <div className='p-4'>
            <BackButton />
            <h1 className='text-3xl text-custom-snow text-center my-4'>Create Book</h1>
            {loading ? <Spinner /> : ''}
            <div className='flex flex-col border-2 border-custom-snow rounded-xl w-[600px] p-4 mx-auto'>
                ...
                <button className='p-2 rounded-lg bg-custom-grey text-custom-snow uppercase' onClick={handleSaveBook}>Add book</button>
            </div>
        </div>
    )
}
